    ...
    <div className="container mx-auto px-4 py-12 md:py-20">
      <div className="text-center mb-16 max-w-2xl mx-auto">
        ...
        <p className="text-muted-foreground">
          {language === 'ar'
            ? 'اكتشف نصائحنا وأخبارنا وقصص عشاق الحيوانات'
            : 'Découvrez nos conseils, actualités et histoires de passionnés.'}
        </p>
      </div>

      {articles.length === 0 ? (
        <div className="text-center py-24 text-muted-foreground">
          {language === 'ar' ? 'لا توجد مقالات بعد' : 'Aucun article pour le moment.'}
        </div>
      ) : (
            ...
            return (
              ...
              >
                <Link href={`/article/${article.id}`} className="group block h-full">
                  <article className="relative h-96 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500">
                    ...
                    <div className="absolute bottom-0 inset-x-0 p-6 flex flex-col justify-end text-white">
                      <div className="flex items-center gap-2 text-xs font-medium text-white/70 mb-3 uppercase tracking-wider">

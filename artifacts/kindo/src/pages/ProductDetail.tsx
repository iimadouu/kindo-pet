      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-24">
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
              {product.images.map((img, idx) => (
                <button
                  ...
                  className={`relative w-20 h-20 rounded-lg overflow-hidden shrink-0 border-2 transition-colors ${activeImage === idx ? 'border-primary' : 'border-transparent'}`}
                >
        ...
        <div className="flex flex-col">
          ...
          <div className="text-3xl font-bold text-primary mb-8 flex items-end gap-2">
            {product.price.toLocaleString('fr-DZ')} <span className="text-lg font-normal mb-1">{t('common.currency')}</span>
          </div>
                ...
                {Object.entries(product.specs).map(([key, value]) => {
                  ...
                  return (
                    <div key={key} className="flex justify-between py-2 border-b border-border/50 text-sm">
                      ...
                      <span className="font-medium text-foreground text-right">{displayVal}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="mt-auto pt-8 flex flex-col sm:flex-row gap-4">
            <Button 
              size="lg" 
              className="flex-1 bg-[#25D366] hover:bg-[#128C7E] text-white h-14 text-base"
              asChild
            >
            ...
            <Button size="lg" variant="outline" className="h-14 w-14 shrink-0 p-0">
